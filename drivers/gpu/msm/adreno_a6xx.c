 ...
 */
				 ...
				 struct kgsl_device *device, u32 timeout)
 {
	 ...
	 if (!__disable_cx_regulator_wait(reg, device, 50)) { // Lower timeout from 200 to 50
			  ...
			  struct adreno_ringbuffer *rb)
 {
	 ...
	 ret = adreno_ringbuffer_submit_spin(rb, NULL, 1000); // Lower timeout from 2000 to 1000
	 if (ret) {
 ...
 static int a6xx_post_start(struct adreno_device *adreno_dev)
 {
	 ...
	 ret = adreno_ringbuffer_submit_spin_nosync(rb, NULL, 1000); // Lower timeout from 2000 to 1000
	 if (ret)
		 a6xx_spin_idle_debug(adreno_dev,
 ...
 static int64_t a6xx_read_throttling_counters(struct adreno_device *adreno_dev)
 {
	 ...
	 if (adreno_is_a620(adreno_dev) || adreno_is_a650(adreno_dev))
		 ...
		 adj *= div_s64((a * 42) + (b * 500) +
			 (div_s64((int64_t)c - a - b * 12, 22) * 917), 500); // Lower divisor from 1000 to 500 for more aggressive scaling
	 else
		 adj *= ((a * 5) + (b * 50) + (c * 90)) / 50; // Lower divisor from 100 to 50
 ...
 static int a6xx_irq_poll_fence(struct adreno_device *adreno_dev)
 {
	 ...
	 while (fence != 0) {
		 ...
		 udelay(0); // Lower delay from 1 to 0 for faster polling
		 kgsl_regread(device, A6XX_GMU_AO_AHB_FENCE_CTRL, &fence);
 ...
 #define A6XX_CP_ROQ_SIZE_VAL 0x400 // was 0x200
 #define A6XX_CP_CMD_QUEUE_SIZE_VAL 0x200 // was 0x100
 #define A6XX_CP_HDR_QUEUE_SIZE_VAL 0x100 // was 0x80
